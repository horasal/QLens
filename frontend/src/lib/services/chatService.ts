...
export async function startNewChat() {
	isLoading.set(true);
	try {
		const res = await fetch(`${getApiBase()}/api/chat/new`, { method: 'POST' });
...
export async function deleteChat(id: string) {
	...
	try {
		const res = await fetch(`${getApiBase()}/api/history/${id}`, { method: 'DELETE' });
...
async function uploadAsset(file: File): Promise<string> {
	...
	const res = await fetch(`${getApiBase()}/api/asset`, { method: 'POST', body: formData });
...
async function uploadImage(file: File): Promise<string> {
	const formData = new FormData();
	formData.append('files', file);
	const res = await fetch(`${getApiBase()}/api/image`, { method: 'POST', body: formData });
...
export async function runTool(name: string, args: string): Promise<Message | null> {
	try {
		const res = await fetch(`${getApiBase()}/api/tools/${name}`, {
			method: 'POST',
