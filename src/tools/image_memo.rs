...
struct ImageMemoArgs {
    #[schemars(description = "String of SVG source code")]
    svg: String,

    #[schemars(
        description = "The optional uuid of the canvas, must be firstly generated by this ImageMemo tool. Set to null to draw on an empty canvas"
    )]
...
impl Tool for ImageMemoTool {
    ...
    fn description(&self) -> ToolDescription {
        ToolDescription {
            ...
            description_for_model: "Render and save memos from svg string to an image canvas."
                .to_string(),
    ...
    fn call(&self, args: &str) -> Result<MessageContent, anyhow::Error> {
        ...
        let uuid = save_image_to_db(&self.db, &img)?;
        Ok(MessageContent::ImageRef(
            uuid,
            args.label.unwrap_or("".to_string()),
        ))
    }
}
...
fn render_svg(canvas: Option<&[u8]>, svg_data: &str) -> Result<Vec<u8>, anyhow::Error> {
    let mut base_image: RgbaImage = match canvas {
        Some(png_data) => image::load_from_memory(png_data)?.to_rgba8(),
        None => RgbaImage::from_pixel(DEFAULT_WIDTH, DEFAULT_HEIGHT, Rgba([0, 0, 0, 0])),
    };
    ...
    let mut svg_pixmap =
        tiny_skia::Pixmap::new(width, height).ok_or(anyhow!("无法创建 tiny_skia::Pixmap"))?;
